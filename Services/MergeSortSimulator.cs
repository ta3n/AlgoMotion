...
public static class MergeSortSimulator
{
    ...
    public static List<SortStep> Record(IReadOnlyList<int> input)
    {
        ...
        if (n > 1) Split(a, 0, n - 1, n, steps, sorted, ref compareCount, ref writeCount);
    ...
    private static void Split(
        int[] a, int lo, int hi, int n,
        List<SortStep> steps, SortedSet<int> sorted,
        ref int compareCount, ref int writeCount)
    {
        if (lo >= hi) return;
    ...
    private static void Merge(
        int[] a, int lo, int mid, int hi, int n,
        List<SortStep> steps, SortedSet<int> sorted,
        ref int compareCount, ref int writeCount)
    {
        ...
        for (var x = 0; x < k; x++) a[lo + x] = tmp[x];
        var mergedSnapshot = a.ToArray();

        for (var x = 0; x < k; x++)
        {
            writeCount++;
            if (isTopLevel) sorted.Add(lo + x);
