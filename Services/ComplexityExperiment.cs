...
public sealed record ComplexitySample(int N, int Comparisons, int Writes)
{
...
{
  public static int[] Input(int n, string pattern)
  {
    var values = Enumerable.Range(1, n).ToArray();
    if (pattern == "Descending") Array.Reverse(values);
    if (pattern == "Random")
    {
...
#pragma warning restore S2245
  ...
  public static ComplexitySample Measure(SortAlgorithmKind kind, int n, string pattern)
  {
    var step = SortAlgorithms.Get(kind).Record(Input(n, pattern), UiLanguage.En)[^1];
    return new(n, step.CompareCount, step.SwapCount);
  }

  public static double Growth(string curve, int n) => curve switch
  {
    "n" => n,
